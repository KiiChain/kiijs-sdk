...
const {
  ethsecpAccountParser,
  signWithEthsecpSigner,
} = require('@kiichain/kiijs-utils');
...
async function evmExample() {
  ...
  try {
    ...
    console.log(
      `Preparing to send ${ethers.formatEther(tx.value)} ETH to ${recipientAddress}`
    );
